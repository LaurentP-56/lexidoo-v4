<nav x-data="{ open: false }" class="bg-cyan-100/50 dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <x-nav-link :href="route('dashboard')" :active="request()->routeIs('dashboard')" class="px-6">
                    {{ __('Tableau de bord') }}
                </x-nav-link>
                <!-- Liens pour la gestion des utilisateurs -->
                <x-nav-link :href="route('admin.users.index')" :active="request()->routeIs('admin.users.*')" class="px-6">
                    {{ __('Gestion des utilisateurs') }}
                </x-nav-link>
                <!-- Liens pour la gestion des tarifs -->
                <x-nav-link :href="route('admin.tarifs.index')" :active="request()->routeIs('admin.tarifs.*')" class="px-6">
                ...
                <div class="hidden sm:flex sm:items-center sm:ml-6">
                    <x-dropdown align="right" width="48">
                        ...
                        <x-slot name="content">
                            ...
                            <x-dropdown-link :href="route('admin.theme.index')" :active="request()->routeIs('admin.theme.*')">
                                {{ __('Gestion des thèmes') }}
                            </x-dropdown-link>
                            <x-dropdown-link :href="route('admin.categories.index')" :active="request()->routeIs('admin.categories.*')">
                                {{ __('Gestion des catégories') }}
                            </x-dropdown-link>
                            <x-dropdown-link :href="route('admin.sub_category.index')" :active="request()->routeIs('admin.sub_category.*')">
                                {{ __('Gestion des sous-catégories') }}
                            </x-dropdown-link>
                            <x-dropdown-link :href="route('admin.mots.index')" :active="request()->routeIs('admin.mots.*')">
                                {{ __('Gestion des mots') }}
                            </x-dropdown-link>
                        </x-slot>
                    </x-dropdown>
                    ...
                    <x-dropdown align="right" width="48">
                        <x-slot name="trigger">
                            <button
                                ...
                                <div><img
                                        src="https://api.dicebear.com/7.x/shapes/svg?seed={{ Auth::user()->email }}&fontFamily=Garamond"
                                        class="h-10 w-10 rounded-full mb-2"
                                        alt="Avatar {{ Auth::user()->prenom }} {{ Auth::user()->nom }}"></div>
                        ...
                        <x-slot name="content">
                            ...
                            <x-dropdown-link :href="route('jeu.index')">
                                {{ __('Mon Jeu') }}
                            </x-dropdown-link>
                            <!-- Authentication -->
